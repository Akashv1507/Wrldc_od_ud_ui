...
class SectionRrasScedDataFetcher():
    ...
    def makeApiCall(self, dateKey:dt.datetime, parameterApiName:str, parameterName):
    ...
    def fetchDataWbesApi(self, startDate:dt.datetime, endDate:dt.datetime):
        ...
        while currDate<=endDate:
            scedData = self.makeApiCall(currDate, 'VSCED_WR', 'SCED')
            rrasData = self.makeApiCall(currDate, 'VAE_WR', 'RRAS')
            timestampList = self.generateTimestampDay(currDate)
            scedListTuple= self.zipTwoList(timestampList, scedData)
